...
if __name__ == '__main__':
    ...
    anno_train_set = dataset(Config = Config,\
                        anno = Config.train_anno,\
                        unswap = transformers["None"],\
                        swap = transformers["None"],\
                        totensor = transformers["val_totensor"],\
    ...
    dataloader['anno_train'] = torch.utils.data.DataLoader(anno_train_set,\
                                                batch_size=args.train_batch,\
                                                shuffle=False,\
                                                #False if args.weighted_sample else True,\
                                                #sampler=DistributedSampler(train_set), #train_set.get_weighted_sampler() if args.weighted_sample else None,\
    ...
    setattr(dataloader['anno_train'], 'total_item_len', len(anno_train_set))
    ...
    as_model.cuda()
    ddp_as_model = nn.DataParallel(as_model)
    #ddp_as_model = DDP(as_model, device_ids=device_ids)
    ...
    filter_optim = optim.SGD([
                               {'params': ddp_as_model.module.parameters()},
                               #{'params': ddp_as_model.parameters(), 'lr': 0.01},
    ...
    #exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[120, 180, 240, 300], gamma=0.1)
    #filter_exp_lr_scheduler = lr_scheduler.MultiStepLR(filter_optim, milestones=[120, 180, 240], gamma=0.1)
    filter_exp_lr_scheduler = lr_scheduler.MultiStepLR(filter_optim, milestones=[400], gamma=0.1)
    #filter_exp_lr_scheduler = lr_scheduler.StepLR(filter_optim, step_size=60, gamma=0.1)
    ...
    models_transfer_dict['bank'] = ddp_as_model
