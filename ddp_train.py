...
if __name__ == '__main__':
    ...
    if args.dataset == 'imagenet':
        aug_prefix = 'imagenet_'
    else:
        aug_prefix = ''

    train_set = dataset(Config = Config,\
                        anno = Config.train_anno,\
                        unswap = transformers[aug_prefix + "unswap"],\
                        swap = transformers["swap"],\
                        totensor = transformers[aug_prefix + "train_totensor"],\
                        train = True)
    ...
    base_params = filter(lambda p: id(p) not in ignored_params, ddp_model.parameters())
    ...
    filter_optim = optim.SGD([
                               {'params': ddp_as_model.module.parameters(), 'lr': 0.08},
    ...
    filter_exp_lr_scheduler = lr_scheduler.StepLR(filter_optim, step_size=30, gamma=0.1)
